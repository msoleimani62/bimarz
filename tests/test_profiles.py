...
برای باینری‌های جعلی xray_manager استفاده شده.
"""
...
def test_file_on_disk_never_contains_plaintext_secrets(
    tmp_path: Path,
) -> None:
    path = tmp_path / "profiles.enc.json"
...
def test_wrong_password_is_rejected(tmp_path: Path) -> None:
    ...
    ProfileStore(
        path=path,
        password="correct-password",
    ).add_profile(_make_profile())

    wrong_store = ProfileStore(
        path=path,
...
def test_listing_a_nonexistent_store_returns_empty(
    tmp_path: Path,
) -> None:
    store = ProfileStore(
        path=tmp_path / "does-not-exist.enc.json",
...
def test_profile_store_creates_private_file(
    tmp_path: Path,
) -> None:
    ...
    store.add_profile(_make_profile("private"))

    assert store_path.exists()
    assert store_path.stat().st_mode & 0o777 == 0o600


def test_profile_store_rejects_empty_password(
    tmp_path: Path,
) -> None:
    with pytest.raises(
        ProfileStoreError,
...
def test_profile_store_rejects_corrupted_json(
    tmp_path: Path,
) -> None:
    store_path = tmp_path / "profiles.json"
